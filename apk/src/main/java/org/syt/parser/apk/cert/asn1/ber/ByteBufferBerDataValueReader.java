...
import org.syt.parser.apk.cert.asn1.ber.BerDataValueReader;
import org.syt.parser.apk.cert.asn1.ber.BerEncoding;

import java.nio.ByteBuffer;

/**
 * {@link org.syt.parser.apk.cert.asn1.ber.BerDataValueReader} which reads from a {@link ByteBuffer} containing BER-encoded data
...
public class ByteBufferBerDataValueReader implements BerDataValueReader {
    ...
    @Override
    public BerDataValue readDataValue() throws org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException {
        ...
        if (!mBuf.hasRemaining()) {
            throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException("Missing length");
        }
    ...
    private int readTagNumber(byte firstIdentifierByte) throws org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException {
    ...
    private int readHighTagNumber() throws org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException {
        ...
        do {
            if (!mBuf.hasRemaining()) {
                throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException("Truncated tag number");
            }
            b = mBuf.get();
            if (result > Integer.MAX_VALUE >>> 7) {
                throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException("Tag number too large");
            }
            result <<= 7;
            result |= b & 0x7f;
        } while ((b & 0x80) != 0);
    ...
    private int readLongFormLength(int firstLengthByte) throws org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException {
        ...
        if (byteCount > 4) {
            throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException("Length too large: " + byteCount + " bytes");
        }
        int result = 0;
        for (int i = 0; i < byteCount; i++) {
            if (!mBuf.hasRemaining()) {
                throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException("Truncated length");
            }
            int b = mBuf.get();
            if (result > Integer.MAX_VALUE >>> 8) {
                throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException("Length too large");
            }
            result <<= 8;
            result |= b & 0xff;
        }
    ...
    private void skipDefiniteLengthContents(int contentsLength) throws org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException {
        if (mBuf.remaining() < contentsLength) {
            throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException(
    ...
    private int skipPrimitiveIndefiniteLengthContents() throws org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException {
        ...
        while (true) {
            ...
            bytesRead++;
            if (bytesRead < 0) {
                throw new org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException("Indefinite-length contents too long");
            }
            if (b == 0) {
                if (prevZeroByte) {
    ...
    private int skipConstructedIndefiniteLengthContents() throws org.syt.parser.apk.cert.asn1.ber.BerDataValueFormatException {
